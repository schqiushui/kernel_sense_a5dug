...
void rmnet_bam_print_packet(const struct sk_buff *skb, const char *dev)
{
	char buffer[200];
	...
	pr_err("[%s]- PKT skb->len=%d skb->head=%p skb->data=%p skb->tail=%p skb->end=%p",
		dev, skb->len, skb->head, skb->data, skb->tail, skb->end);
	...
	pr_err("[%s] - PKT len: %d, printing first %d bytes",
		dev, len, printlen);

	memset(buffer, 0, sizeof(buffer));
	for (i = 0; (i < printlen) && (i < len); i++) {
		if ((i%16) == 0) {
			pr_err("[%s]- PKT%s", dev, buffer);
			memset(buffer, 0, sizeof(buffer));
			buffloc = 0;
...
#define RMNET_BAM_MAGIC_MARK   0xFFFFFFFF
...
static int _rmnet_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (RMNET_IS_MODE_QOS(opmode)) {
		...
		if (skb->mark == RMNET_BAM_MAGIC_MARK) {
			rmnet_bam_print_packet(skb, dev->name);  
		}
	}
